...
import { PATHS, RESOURCES, awayGameMapper, awayGameToEvent, makeRequest, upsertBus, upsertEvent, upsertMembership, wpMembershipToMembership } from "~/server/utils/cron";
...
export const cronRouter = createTRPCRouter({
  ...
  syncMemberships: cronProcedure
    .mutation(async ({ ctx }) => {
      console.info("Syncing memberships");
      const res = await makeRequest<Membership[]>(PATHS.wpURL + RESOURCES.membership, 'GET');
      const memberships = res.flatMap(wpMembershipToMembership);
      await Promise.all(memberships.map((membership) => upsertMembership(membership, ctx)));
