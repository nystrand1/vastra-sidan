...
import {
  bandyDivisions,
  fotballDivisions
} from "~/server/utils/awayGuideSorter";
import {
  GetAboutUsDocument,
  GetAwayGuideBySlugDocument,
  GetAwayGuidesDocument,
  GetChronicleDocument,
  GetChroniclesDocument,
  GetNewsBySlugDocument,
  GetNewsDocument,
  GetSeasonChroncilesDocument,
  GetSeasonChronicleBySlugDocument
...
export const wordpressRouter = createTRPCRouter({
  getChronicles: publicProcedure.query(async ({ ctx }) => {
    ...
    return res.data.chronicles.nodes.map((chronicle) => ({
      ...chronicle,
      excerpt:
        stripHtmlTags(chronicle.chronicleFields.text)
          .split(".")
          .splice(0, 3)
          .join(" ") + ".",
  ...
  getAwayGuides: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const fotballGuides = guides.filter((guide) =>
        guide.awayGuideFields.sport.includes("Fotboll")
      );
      const bandyGuides = guides.filter((guide) =>
        guide.awayGuideFields.sport.includes("Bandy")
      );

      const fotballByDivision = fotballDivisions
        .map((division) => {
          return {
            division,
            guides: fotballGuides.filter(
              (guide) => guide.awayGuideFields.division === division
            )
          };
        })
        .filter((division) => division.guides.length > 0);

      const bandyByDivision = bandyDivisions
        .map((division) => {
          return {
            division,
            guides: bandyGuides.filter(
              (guide) => guide.awayGuideFields.division === division
            )
          };
        })
  ...
  getNews: publicProcedure.query(async ({ ctx }) => {
    ...
    return res.data.newsPosts.nodes.map((news) => ({
      ...
      excerpt:
        stripHtmlTags(news.newsContent.text).split(".").splice(0, 4).join(" ") +
        ".",
      date: parseDateString(news.date),
      image: news.newsContent.newsImg?.node,
  ...
  getNewsBySlug: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return {
        ...
        image: newsPost.newsContent.newsImg?.node,
        author: newsPost.newsContent.author
      };
    }),
  getSeasonChronicles: publicProcedure.query(async ({ ctx }) => {
    const { data } = await ctx.apolloClient.query({
    ...
    while (hasNextPage) {
      ...
      seasonChronicles = [
        ...seasonChronicles,
        ...nextPage.data.seasonChronicles.nodes
      ];
    ...
    const bandySeasonChronicles = seasonChronicles
      .filter((chronicle) =>
        chronicle.seasonChronicleContent.sport.includes("Bandy")
      )
      .sort((a, b) =>
        b.seasonChronicleContent.title.localeCompare(
          a.seasonChronicleContent.title
        )
      );
    const fotballSeasonChronicles = seasonChronicles
      .filter((chronicle) =>
        chronicle.seasonChronicleContent.sport.includes("Fotboll")
      )
      .sort((a, b) =>
        b.seasonChronicleContent.title.localeCompare(
          a.seasonChronicleContent.title
  ...
  getAboutUsPage: publicProcedure.query(async ({ ctx }) => {
    const { data } = await ctx.apolloClient.query({
  ...
  getWallOfFamePage: publicProcedure.query(async ({ ctx }) => {
    const { data } = await ctx.apolloClient.query({
      query: GetAboutUsDocument
    });
